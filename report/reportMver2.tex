...
本プログラムは、ユーザの選択がステータスと未来のイベントに影響を与える「状態遷移型」のシミュレーションゲームとして設計した。

最も特徴的なアルゴリズムは\textbf{「可変時間ステップ」}である。選択肢を選んだとき、すべての選択肢で経過時間を等しくするのではなく、選択した行動の重みに応じて経過時間を動的に変化させるデータ構造を採用した。
...
View（表示）やController（入力）から独立させることで、仕様変更に強い設計を目指した。
...
\textbf{工夫した点：}
\begin{itemize}
    \item \textbf{データの整合性保証}: 単に値を足し引きするだけでなく、上限（100）や下限（0）のチェックをメソッド内で行うことで、体力が100を超えてしまうなどの異常なステータスが発生しないように設計した。
    \item \textbf{可変時間への対応}: 年齢の加算を固定値ではなく引数で受け取る仕様にし、イベントごとの時間経過の違いに対応させた。
\end{itemize}

\begin{lstlisting}[caption=Player.java（ステータス操作の抜粋）, label=code:player_detail]
public class Player {
    // 状態フィールド（外部から直接変更不可）
    private int age;
    private int health;
...
\begin{lstlisting}[caption=Choice.java（データ構造）, label=code:choice_detail]
class Choice {
...
\subsubsection{GameLogicクラス（ゲームロジック）}
...
\begin{lstlisting}[caption=GameLogic.java（イベント生成の抜粋）, label=code:logic_detail]
class GameLogic {
...
\subsubsection{GameModelクラス / DeathResultクラス（統合・ファサード）}
...
\begin{lstlisting}[caption=GameModel.java（ファサードの実装詳細）, label=code:gamemodel_detail]
class GameModel {
...
View層は、ユーザーへの情報の提示と操作の受け付けを担当する部分である。
Java標準のGUIツールキットであるSwingを使用し、リッチなグラフィカルインターフェースを構築した。
主なクラスとして、タイトル画面を管理する\texttt{TitleView}、メインゲーム画面の\texttt{GameView}、結果表示の\texttt{ResultView}を作成した。

\subsubsection{GameViewクラス（メイン画面）}
ゲームの進行状況やイベントを表示する主要な画面である。
画面構成は上から順に、現在のステータスを表示するヘッダーパネル、イベントテキストと結果ログを表示するセンターパネル、そして選択肢ボタンを配置するフッターパネルの3部構成となっている。

\textbf{工夫した点：}
\begin{itemize}
    \item \textbf{視覚的なフィードバック}: ステータス（体力・ストレス・所持金）が増減した際、数値の変化だけでなく、\texttt{javax.swing.Timer}を利用してプログレスバーやラベルの色を一瞬変化させるアニメーション処理を実装した。これにより、プレイヤーは直感的に「ダメージを受けた」「回復した」といった変化を感じ取ることができる。
    \item \textbf{HTMLによるリッチテキスト表示}: \texttt{JButton}や\texttt{JLabel}はHTMLタグを解釈できる機能を持っている。これを利用し、選択肢ボタンの中に太字の行動内容と、小さく色を変えた補足情報（「(+3年)」など）を併記することで、情報量を増やしつつ見やすさを維持した。
\end{itemize}

\begin{lstlisting}[caption=GameView.java（視覚効果の実装）, label=code:view_flash]
    // ステータス変化時にバーを一瞬点滅させるメソッド
    private void flashBar(JProgressBar bar, Color flashColor) {
        Color original = bar.getForeground();
        bar.setForeground(flashColor);
        // 200ミリ秒後に元の色に戻すタイマーを作成
        Timer timer = new Timer(200, e -> bar.setForeground(original));
        timer.setRepeats(false); // 1回だけ実行
        timer.start();
    }

    // 選択肢ボタンのテキスト生成（HTML使用）
    private String formatChoiceText(Choice choice, ChoiceEffect effect) {
        // ... (省略) ...
        return "<html><b>" + escapeHtml(choice.getText()) + "</b><br>" +
               "<span style='color:#888888; font-size:10px;'>" +
               escapeHtml(effectText) + "</span></html>";
    }
\end{lstlisting}

\subsubsection{TitleView / ResultView（画面遷移）}
ゲームの開始と終了を担当する画面である。
\texttt{TitleView}では、難易度設定やプレイヤー名の入力フォームを作成した。
\texttt{ResultView}では、ゲーム終了時の詳細なリザルトを表示する。ここでは\texttt{JScrollPane}を使用し、長いプレイ履歴でもスクロールして全て確認できるようにした。

また、全体を通して配色には「Dark Mode」を意識した暗めの背景色（\texttt{\#282C34}など）を採用し、長時間プレイしても目が疲れにくいモダンなデザインを目指した。
...
最も苦労したのは、ViewやControllerから「いかにして内部ロジックを隠蔽するか」という点である。当初はControllerから直接\texttt{Player}の値を書き換えるような設計を考えていたが、それだとバグの原因になりやすいと気付き、\texttt{GameModel}という窓口クラスを用意して「選択肢を渡せば結果が返ってくる」というシンプルなインターフェースに徹した。
結果として、チームメンバーとの連携がスムーズに進み、MVCモデルの利点（分業のしやすさ）を肌で感じることができた。オブジェクト指向設計の重要性を再認識する良い機会となった。

\subsection{佐藤 〇〇（View担当）}
私はView層を担当し、主にSwingを用いたGUIの構築と、ゲーム画面の描画処理を行った。
初めはSwing特有のレイアウトマネージャ（BorderLayoutやBoxLayoutなど）の挙動を理解するのに苦労し、意図した通りの画面配置にするため試行錯誤を繰り返した。特に、コンポーネントの隙間調整や、ウィンドウサイズが変化しても崩れないレイアウトに調整する点は難易度が高かった。
また、Model層のデータ（体力やストレス値など）をどのようにリアルタイムで画面に反映させるか悩んだが、最終的にControllerから\texttt{updateDisplay}メソッドを通じて描画を更新する仕組みを採用し、MVCパターンのメリットである「表示とロジックの分離」を実装を通して理解することができた。
工夫点として挙げた「ステータス変動時のフラッシュ演出」や「HTMLタグを用いたリッチなボタン」が実際に動作し、ゲームらしい表現ができた時は大きな達成感があった。
チーム開発という観点でも、担当領域を明確に分けたことで並行作業がスムーズに進み、統合時にも大きな混乱なく動作させることができた良い経験となった。
...
なお、コンパイルおよび実行には JDK 8 以上が必要である。

\subsection*{1. エントリーポイント (Main.java)}
...
public class Main {
    public static void main(String[] args) {
        // SwingアプリはEDT(Event Dispatch Thread)で起動する
...
\begin{lstlisting}[caption=GameModel.java（統合ファサード）]
/**
 * Model層の窓口クラス。Controllerからの要求を一手に引き受ける。
 */
class GameModel {
