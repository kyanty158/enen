...
初めはSwing特有のレイアウトマネージャであるBorderLayoutやBoxLayoutなどの挙動を理解するのに苦労し、意図した通りの画面配置にするため試行錯誤を繰り返した。特に、コンポーネントの隙間調整や、ウィンドウサイズが変化しても崩れないレイアウトに調整する点は難易度が高かった。
また、Model層の体力やストレス値などのデータをどのようにリアルタイムで画面に反映させるか悩んだが、最終的にControllerから\texttt{updateDisplay}メソッドを通じて描画を更新する仕組みを採用し、MVCパターンのメリットである表示とロジックの分離を実装を通して理解することができた。
工夫点として挙げたステータス変動時のフラッシュ演出やHTMLタグを用いたリッチなボタンが実際に動作し、ゲームらしい表現ができた時は大きな達成感があった。
